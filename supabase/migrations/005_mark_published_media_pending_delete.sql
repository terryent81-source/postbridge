-- Re-apply the media cleanup marker RPC for projects that already ran 004.
--
-- Storage remains temporary SNS upload staging. This RPC only marks rows for
-- later cleanup; it never removes Storage objects. Cleanup workers/server
-- routes must use service_role to call Storage remove and then mark rows
-- deleted.
...
declare
  ...
  normalized_delete_after timestamptz := coalesce(delete_after_at, now());
begin
  ...
  set
    ...
    delete_after = normalized_delete_after
